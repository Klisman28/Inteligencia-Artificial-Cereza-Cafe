...
def circle_contour(image, contour):
    ...
    #recortar(imagenConElipse,y,(y + sx*2),x,(x + sy*2))

    #cv2.ellipse(imagenConElipse, ellipse, green, 2, cv2.LINE_AA)
    #cv2.rectangle(imagenConElipse, (x,y), ((x + sy*2), (y + sx*2)), (255,0,0),2)
    """
    cols, rows, other = imagenConElipse.shape
    M = cv2.getRotationMatrix2D((cols/2,rows/2),int(ellipse[2]) + 270,1)
    imagenConElipse = cv2.warpAffine(imagenConElipse,M,(rows,cols))
    """
